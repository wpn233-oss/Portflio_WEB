...
export function LeftPanel({ project, onBack }: LeftPanelProps) {
  ...
  return <div className="left-content">
    <header className="identity"><button className="name-button" onClick={project ? onBack : undefined}><span>熊政</span>{project && <i>项目集</i>}</button><p>Visual &amp; product designer</p></header>
    {project ? <section className="project-sidebar">
      ...
      <FoldableButton label="分享" open={open === 'share'} onClick={() => toggle('share')}>{project.documentUrl ? <a className="share-button" href={project.documentUrl} target="_blank" rel="noopener noreferrer">打开项目 PDF <span>↗</span></a> : <button className="share-button" onClick={share}>复制项目链接 <span>↗</span></button>}</FoldableButton>
    </section> : <section className="home-sidebar">
      <div className="icon-dock">{dock.map(({ key, label, icon: Icon }) => <button key={key} className={open === key ? 'selected' : ''} onClick={() => toggle(key)} aria-label={label}><Icon size={18} strokeWidth={1.55} /></button>)}</div>
      <FoldableButton label="联系方式" open={open === 'contact'} onClick={() => toggle('contact')}><a href="mailto:2131495751@qq.com">2131495751@qq.com</a><span>微信号：wpn-233</span></FoldableButton>
      <FoldableButton label="个人介绍" open={open === 'about'} onClick={() => toggle('about')}><p>我是一名专注于数字体验与品牌表达的设计师。喜欢把复杂的需求，变成简洁、清晰，也带一点温度的界面。</p></FoldableButton>
    </section>}
